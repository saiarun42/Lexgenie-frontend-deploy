...
const BnsSearch: React.FC = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await axios.get(`http://13.61.2.47:80/recent-files/${folderName}`);
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      ...
      const response = await axios.post("http://13.61.2.47:80/ipc-classifier", formData);
      ...
      setDocuments((prev) => [
        ...prev,
        {
          ...
          name: files[0].name,
          url: `http://13.61.2.47:80${response.data.view_url}`,
          content: content
        },
      ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://13.61.2.47:80/continue-conversation-ipc", payload);
      setMessages((prev) => [
        ...prev, 
        { 

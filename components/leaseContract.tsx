...
const LeaseForm: React.FC = () => {
  ...
  useEffect(() => {
    setMessages([
      {
        type: "question",
        content: "Who is the Landlord (party_a)?",
      },
    ]);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("http://13.61.2.47:80/lease_process_response", {
        method: "POST",
      ...
      if (data.lease_agreement) {
        setGenerating(true);
        setTimeout(() => {
          setLeaseAgreement(data.lease_agreement || "string");
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch(`http://13.61.2.47:80/download-document/${documentId}`);
      ...
      a.href = url;
      a.download = "lease_Agreement.doc";
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      window.URL.revokeObjectURL(url);

      console.log("Downloaded lease Agreement successfully.");
    } catch (error) {
      ...
      <div className="max-w-4xl mx-auto mt-10">
        <div className="bg-white rounded-t-lg shadow-sm border border-gray-200 p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold text-gray-900">Lease Agreement Generator</h2>
          </div>
        </div>
        {generating && (
          <div className="flex flex-col items-center justify-center mt-5">
            <p className="text-lg font-semibold text-gray-600">Generating your Lease agreement...</p>
          ...
          <div className="space-y-4">
            {messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.type === "answer" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[80%] rounded-lg p-3 ${msg.type === "answer" ? "bg-gray-300 text-white" : msg.type === "error" ? "bg-red-100 text-red-700" : "bg-gray-400 text-white"}`} style={{color:'white'}}>
        ...
        <div className="bg-white rounded-b-lg shadow-sm border border-t-0 border-gray-200 p-4">
          <form onSubmit={handleSubmit} className="flex gap-4">
            ...
            />
            <button type="submit" disabled={loading || !message} className="inline-flex items-center gap-2 rounded-lg bg-gray-700 px-4 py-2 text-white hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
        ...
        {leaseAgreement && (
          <div className="mt-6 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={handleDownload} disabled={!documentId} className="inline-flex items-center gap-2 bg-black px-4 py-2 text-white hover:bg-gray-700 rounded-lg">
        ...
        <div className="mt-4 p-4 border border-gray-200 rounded-lg bg-gray-50 max-h-[400px] overflow-y-auto">
              <div className="prose prose-sm">
                {safeFormatResponse(leaseAgreement)}
              </div>
            </div>
      </div>
    </div>

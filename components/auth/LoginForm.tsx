...
const LoginForm: React.FC<LoginFormProps> = ({ callbackUrl }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.replace(callbackUrl); // Immediately go to dashboard
    } catch (err: any) {
  ...
  return (
    <div className="relative z-10 w-full max-w-md p-8 rounded-lg backdrop-blur-sm bg-white/20 shadow-xl border border-white/30">
      ...
      {error && (
        <div className={`mb-4 p-3 rounded-md text-center ${error.includes('successful') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button type="submit" disabled={loading} className="w-full py-2 px-4 rounded-md text-sm font-medium text-black bg-gray-100 hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed">
          {loading ? 'Signing in...' : 'Sign in'}
        </button>
      </form>

      <p className="mt-6 text-center text-sm">
        Don&apos;t have an account? <Link href="/signup" className="font-medium hover:text-gray-200">Sign up</Link>
      </p>
    </div>
  );
};

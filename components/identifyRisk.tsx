...
const Layout: React.FC = () => {
  ...
  const [, setIdentifiedRisks] = useState<string[] | null>(null);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const risksRes = await fetch("http://13.61.2.47:80/identify-risks", {
        method: "POST",
        body: formData,
      });

      if (!risksRes.ok) throw new Error("Failed to identify risks");
      ...
      if (Array.isArray(risksData.risks)) {
        setIdentifiedRisks(risksData.risks);
        setChatMessages((prev) => [
          ...prev,
          { text: "Identified Risks:\n" + risksData.risks.join("\n"), fromUser: false },
        ]);
      } else if (typeof risksData.risks === "string") {
        // Handle the case where risks are returned as a string
        setIdentifiedRisks([risksData.risks]);
        setChatMessages((prev) => [
          ...prev,
          { text: "Identified Risks:\n" + risksData.risks, fromUser: false },
        ]);
      } else {
        throw new Error("Risks data format is not recognized.");
      }
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      {/* Left Panel: Identified Risks & Upload */}
      <div className="w-1/5 p-4 bg-white border-r border-gray-300">
        <Returnpage />
        <h2 className="text-xl font-bold mb-4">Identified Risks</h2>
        <div className="mb-6">
          <p>No risks identified yet.</p>

        </div>
        <h2 className="text-xl font-bold mb-4">Upload Document</h2>
        <input type="file" onChange={handleFileChange} className="border p-2 mb-4 w-full" />
        <button onClick={handleUpload} className="w-full bg-black text-white p-2 rounded hover:bg-gray-600">
          Upload
        </button>
      </div>
      ...
      <div className="w-4/5 p-4 flex flex-col">
        <div className="flex-grow bg-white p-4 mb-4 border rounded-md">
          <h2 className="text-xl font-bold mb-2">Indentify Risks</h2>
          {documents.length > 0 && (
            <button
          ...
          <div className="h-96 overflow-y-scroll bg-gray-50 p-2 rounded-lg mb-4">
            {chatMessages.map((msg, index) => (
              <div key={index} className={`p-2 mb-2 ${msg.fromUser ? "bg-blue-100" : "bg-gray-200"} rounded`}>
